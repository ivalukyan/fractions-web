...
@router.get('/task_selection/{email}/{class_id}/mixed_tasks/{task_id}/{correct}/{count_task}')
...
@router.post('/task_selection/{email}/{class_id}/mixed_tasks/{task_id}/{correct}/{count_task}')
async def arithmetic_operations(request: Request, class_id: str, answer: Annotated[str, Form()], task_id: int,
    ...
    if answer_check(answer):
        ...
        db_task = db_session.query(Task).filter(Task.class_student == class_id,
                                                Task.type_task == 'mixed_tasks').all()
